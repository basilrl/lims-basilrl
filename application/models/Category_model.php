...
class Category_model extends MY_Model
{
    ...
    public  function getCategory_list($per_page, $page = 0, $search, $where, $count = NULL)
    {
        ...
        $this->db->order_by('mst_category.category_id', 'DESC');
        if (!empty($where)) {
            $this->db->group_start();
            $this->db->where($where);
            $this->db->group_end();
        ...
        if (!empty($search)) {
            ...
            $this->db->or_like('mst_category.category_code', $search);
            $this->db->group_end();
        }
        $this->db->select('mst_category.*,CONCAT(admin_profile.admin_fname," ",admin_profile.admin_lname) as created_by');
