...
class Operation_model extends MY_Model
{
    ...
    public function get_operation_list($per_page, $page = 0, $search, $where, $count = NULL)
    {
        ...
        if (!empty($search)) {
            $this->db->group_start();
            $this->db->like('functions.controller_name', $search);
            $this->db->or_like('functions.function_name', $search);
            $this->db->or_like('functions.alias', $search);
            $this->db->group_end();
        }
