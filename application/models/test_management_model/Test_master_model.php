...
class Test_master_model extends MY_Model
{
	...
	public function check_dupli($data, $test_id = NULL)
	{
		...
		$this->db->select('test_name,test_method_id');
		$this->db->from('tests');
		$this->db->group_start();
	...
	public function get_test_method($key){
		...
		($key != NULL) ? $this->db->like('tm.test_method_name', $key, 'both') : '';
	...
	public function get_test_details($test_id){
		$this->db->select('test_id, test_name, test_division_id, test_service_type, div.division_name as div_name, test_lab_type_id, lab.lab_type_name as lab_name, minimum_quantity, minimum_quantity_units, min_qty.unit as min_qty_unit, units, report.unit as report_unit, test_turn_around_time, test_status, method_type, tm.test_method_id, tm.test_method_name, is_available_customerportal, under_scope');
