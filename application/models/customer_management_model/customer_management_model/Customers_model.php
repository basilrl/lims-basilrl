...
class Customers_model extends MY_Model
{
  ...
  {
    $this->db->select('cust.*,country.country_name,state.province_name as state_name,location.location_name as location_name,CONCAT(admin_fname," ",admin_lname) as created_by');
    $this->db->from('cust_customers cust');
    $this->db->where('customer_id', $customer_id);
    $this->db->join('mst_country country', 'country.country_id=cust.cust_customers_country_id and country.status="1"', 'left');
  ...
  public function loadOportunity($customer_id){
    $this->db->select('opportunity.opportunity_value as opportunity_value,opportunity.types as types,opportunity.estimated_closure_date as estimated_closure_date,opportunity_id as reference_id,opportunity_name as reference');
  ...
  public function get_map_listing($customer_id=NULL,$type=NULL,$data){
  ...
  public function get_mapped_listing($join_table,$condition,$where1,$where2){
    $this->db->select('cs.customer_id,cs.customer_name,cs.customer_code,cs.city');
    $this->db->from('cust_customers cs');
  ...
  public function op_assign_to($designation_id2,$designation_id1){
    $this->db->select('admin_profile.uidnr_admin as user_id,CONCAT( admin_profile.admin_fname,"",admin_profile.admin_lname) as user_name');
