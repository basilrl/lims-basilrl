...
class Website_Operation extends MY_Controller
{

    public function __construct()
    {
        parent::__construct();   
        $this->load->model('Website_Operation_model','OM'); 
        // $this->permission('Website_Operation/index');    
    ...
    {
        $this->load_view('Website_operation/index');
    }
    public function operation_listing($search,$page)
    {
        ...
        $config['base_url'] = base_url('Website_Operation/operation_listing');
        ...
        if ($result) {
            foreach ($result as $value) {
                ...
                // if ($this->permission_action('Operation/edit')) {
                    $html .= '<a title="EDIT" href="javascript:void(0);" class="btn btn-sm edit_con" data-toggle="modal" data-id="' . base64_encode($value->function_id) . '" data-target="#edit_operation"><img width="28px" src="' . base_url('public/img/icon/edit.png') . '" alt="BASIL" class="edit_application_data" ></a>';
                // }
    ...
    public function add(){
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $insert = $this->OM->insert_data('front_functions', $post);
            if ($insert) {
    ...
    public function operation_get()
    {
        ...
        echo json_encode($this->OM->get_row('*', 'front_functions', ['function_id' => $post['id']]));
    }
    public function edit()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            $post = $this->input->post();
            $get = $this->OM->get_row('alias','front_functions',['function_id'=>$post['function_id']]);
            $insert = $this->OM->update_data('front_functions', $post,['function_id'=>$post['function_id']]);
            if ($insert) {
    ...
    public function log_history($reason)
    {
        $array = array(
            'text'=>$reason['action'],
            'record_id'=>$reason['id'],
            'source_module' => 'Website_Operation',
