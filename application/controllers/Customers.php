...
class Customers extends MY_Controller
{
	...
	public function index()
	{
		...
		$me=$this->customers_model->customers_list(NULL, NULL, $search, $sortby, $order, $where);
		// echo "<pre>";
	...
	public function add_customers()
	{
		...
		if ($this->form_validation->run()) {
			...
			if ($data && count($data) > 0) {
				...
				if ($result) {
					/* Added on22-07-2021 by Kapri To add customer on BC365 system as well */
					if (IS_ERP_INTEGRATED) {
						$this->_checkCustomerOnBC365($result);
					}
	...
	public function export_customers()
	{
		...
		if ($data && count($data) > 0) {
			...
			$objPHPExcel->getActiveSheet()->setCellValue('E1', "Test Name");
			$objPHPExcel->getActiveSheet()->setCellValue('F1', "Perfoma Invoice Amount");
			...
			$objPHPExcel->getActiveSheet()->setCellValue('N1', "PIN CODE");
			$objPHPExcel->getActiveSheet()->setCellValue('O1', "COUNTRY");
			$objPHPExcel->getActiveSheet()->setCellValue('P1', "WEBSITE");
			$objPHPExcel->getActiveSheet()->setCellValue('Q1', "STATUS");
			$objPHPExcel->getActiveSheet()->setCellValue('R1', "CREDIT");
			$objPHPExcel->getActiveSheet()->setCellValue('S1', "PAN NO.");
			$objPHPExcel->getActiveSheet()->setCellValue('T1', "TAN NO.");
			$objPHPExcel->getActiveSheet()->setCellValue('U1', "GSTIN");
			$objPHPExcel->getActiveSheet()->setCellValue('V1', "RETENTION PERIOD");
			...
			foreach ($data as $key => $value) {
				...
				$objPHPExcel->getActiveSheet()->setCellValue('E' . $i, ($value->test_name) ? $value->test_name : '');
				$objPHPExcel->getActiveSheet()->setCellValue('F' . $i, ($value->total_amount) ? $value->total_amount : '');

				$objPHPExcel->getActiveSheet()->setCellValue('G' . $i, ($value->email) ? $value->email : '');
				$objPHPExcel->getActiveSheet()->setCellValue('H' . $i, ($value->created_by) ? $value->created_by : '');
				$objPHPExcel->getActiveSheet()->setCellValue('I' . $i, ($value->created_on) ? $value->created_on : '');

				$objPHPExcel->getActiveSheet()->setCellValue('J' . $i, ($value->telephone) ? $value->telephone : '');

				$objPHPExcel->getActiveSheet()->setCellValue('K' . $i, ($value->mobile) ? $value->mobile : '');
				$objPHPExcel->getActiveSheet()->setCellValue('L' . $i, ($value->address) ? $value->address : '');
				$objPHPExcel->getActiveSheet()->setCellValue('M' . $i, ($value->city) ? $value->city : '');
				$objPHPExcel->getActiveSheet()->setCellValue('N' . $i, ($value->po_box) ? $value->po_box : '');
				$objPHPExcel->getActiveSheet()->setCellValue('O' . $i, ($value->country_name) ? $value->country_name : '');

				$objPHPExcel->getActiveSheet()->setCellValue('P' . $i, ($value->web) ? $value->web : '');

				$objPHPExcel->getActiveSheet()->setCellValue('Q' . $i, ($value->status) ? $value->status : '');

				$objPHPExcel->getActiveSheet()->setCellValue('R' . $i, ($value->credit) ? $value->credit : '');

				$objPHPExcel->getActiveSheet()->setCellValue('S' . $i, ($value->pan) ? $value->pan : '');
				$objPHPExcel->getActiveSheet()->setCellValue('T' . $i, ($value->tan) ? $value->tan : '');
				$objPHPExcel->getActiveSheet()->setCellValue('U' . $i, ($value->gstin) ? $value->gstin : '');
				$objPHPExcel->getActiveSheet()->setCellValue('V' . $i, ($value->retention_period) ? $value->retention_period : '');
				$i++;
			}
	...
	public function download_pdf()
    {
     ...
     public function pushCustomerToBC(){
      $customer_id = $this->input->post('customer_id');
      $customer =  $this->db->select('company_name as customer_name, customer_code, address,'
                                            . ' city, email, credit_limit, country_id as cust_customers_country_id, '
                                            . ' credit_days as credit,'
                                            . ' phone_number as telephone, gstin')
                            ->get_where('customers', ['customer_id' => $customer_id])->row_array();
      if (defined("IS_ERP_INTEGRATED") && IS_ERP_INTEGRATED) {
           $response = $this->_checkCustomerOnBC365($customer);
