...
<style>
     @page {
        /* background: url('<?php echo base_url();?>assets/images/new_images/gurugram.png') no-repeat 0 0; */
        background: url('https://cpslims-prod.s3.ap-south-1.amazonaws.com/format/gurugram-format.png') no-repeat 0 0;
...
<body>
    ...
    <div class="bg_image">
        <htmlpageheader name="myHeader1">
           ...
           <table style="text-align: left;margin-top:70px;width:100%;font-size:12px;">
           <tr>
                <th style="text-align: left;width:80%">Report No.: <?php echo $report_data->report_num; ?></th>
...
<?php if(!empty($report_data->buyer_name)){?>
        ...
        <?php if ($report_data->part == 'yes') { ?>
            ...
            <?php } ?> <br>
        <?php if ($test_data) { ?>
            <table border="1" style="width: 100%;border-collapse:collapse;text-align:center;margin-top:10px;font-size:12px;">
                <thead>
                    <tr>
                        <th>S.No.</th>
          ...
          <?php  if($report_data->remark){?>
            ...
            <table style="width: 100%;padding:0;margin:0;font-size:12px;">
               ...
               <?php if($sign_data1){?>
                ...
                <td>Test result is drawn according to the kind and extent tests performed. <br>
Without permission of the tests centre, this report is not permitted to be duplicated in extracts. This report does not entitle to carry any safety mark on this or similar products. This test report represents the test parameters as requested by the customer based on submitted sample only.
